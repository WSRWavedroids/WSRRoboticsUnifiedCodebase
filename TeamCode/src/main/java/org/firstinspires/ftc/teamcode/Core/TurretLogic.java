    ...
    /// Dont worry about unsafe positions, a different function converts these
    {
        ...
        else if(activeMode.equals(FULL))
        {
            ...
            if(robot.alliance.equals(BLUE))
            {
                ...
                rawAngle = Math.toDegrees(Math.atan2((robot.turretPosition.x - 6), (robot.turretPosition.y - 135)));
            }
            else
            {
                //gets the raw angle without accounting for heading
                rawAngle = Math.toDegrees(Math.atan2((robot.turretPosition.x - 138), (robot.turretPosition.y - 135)));
